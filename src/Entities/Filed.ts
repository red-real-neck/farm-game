import { BlockTextures, Field3D } from "../3dObjects/Field3D";
...
export class Field {
  ...
  constructor(fieldHeight, fieldWidth, textures: BlockTextures) {
    this._fieldHeight = fieldHeight;
    this._fieldWidth = fieldWidth;
    this._createItems(textures);
  }

  private _createItems(textures: BlockTextures) {
    for (let i = 0; i < this._fieldHeight * this._fieldWidth; i++) {
      const filedItem = new FieldItem(textures);
      this._items.push(filedItem);
    }
    this._generateField(textures);
  }

  private _generateField(textures: BlockTextures) {
